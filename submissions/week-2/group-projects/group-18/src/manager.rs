...
impl FeedbackManager {
    ...
    pub fn add_feedback(&mut self, customer_name: String, comment: String, rating: u8) -> Result<(), FeedbackError> {
    ...
    pub fn view_all_feedback(&self) {
        ...
        for feedback in self.feedbacks.values() {
            ...
            println!("Date: {}", feedback.date_created.format("%Y-%m-%d %H:%M:%S UTC"));
            println!("{}", "-".repeat(40));
        }
    }
    ...
    pub fn edit_feedback(&mut self, id: u32) -> Result<(), FeedbackError> {
        let feedback = self.feedbacks.get(&id).ok_or(FeedbackError::FeedbackNotFound)?;
        ...
        let choice: u8 = choice.trim().parse().map_err(|_| FeedbackError::InvalidInput)?;
        
        let mut updated_feedback = self.feedbacks[&id].clone();
        
        match choice {
            1 => {
                let new_name = get_user_input("Enter new customer name: ")?;
                updated_feedback.customer_name = new_name.trim().to_string();
            }
            2 => {
                let new_rating = get_user_input("Enter new rating (1-5): ")?;
                let rating: u8 = new_rating.trim().parse().map_err(|_| FeedbackError::InvalidInput)?;
                if rating < 1 || rating > 5 {
                    return Err(FeedbackError::InvalidRating);
                }
            ...
            3 => {
                let new_comment = get_user_input("Enter new comment: ")?;
